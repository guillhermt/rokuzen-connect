...
async function carregarPostos() {
  ...
  } catch (e) { console.log("Erro postos:", e); }
}
...
async function carregarTerapeutas() {
  try {
    ...
    if (res.ok) {
      ...
      terapeutas = todos.filter(c => c.tipo_colaborador === 3 && c.ativo === 'S');
    }
  } catch (e) { console.log("Erro terapeutas:", e); }
}
...
function renderizarPostos() {
  ...
  grid.innerHTML = postos.map(p => {
    const atendimento = atendimentosAtivos.find(a => a.posto_id == p.posto_id && !a.finalizado);
        ...
        <div class="posto-card posto-${status} h-100">
          <div class="header-posto text-white text-center p-4">
            <div>
              ...
              ${p.terapeuta_nome ? `<small>Terapeuta: ${p.terapeuta_nome}</small>` : ''}
            </div>

            ${atendimento ? `
              <div>
                <div class="cronometro display-4 fw-bold">${formatarTempo(atendimento.tempo_restante || 0)}</div>
                <p class="fs-5 fw-bold mb-3">${atendimento.cliente}</p>
                <button class="btn btn-light btn-lg w-100" onclick="finalizar(${p.posto_id})">
                  FINALIZAR
                </button>
              </div>
            ` : emIntervalo ? `
              <div>
                ...
                <button class="btn btn-light btn-lg w-100" onclick="cancelarIntervalo(${p.posto_id})">
                  CANCELAR INTERVALO
                </button>
              </div>
            ` : emManutencao ? `
...
function renderizarTerapeutas() {
  ...
  const html = terapeutas.map(t => {
    // Procura se esse terapeuta está em algum atendimento ativo
    const atendimentoAtivo = atendimentosAtivos.find(a => 
      !a.finalizado && 
      Number(a.terapeuta_id) === Number(t.colaborador_id)
    );
    ...
    const postoTexto = estaOcupado ? `Posto ${atendimentoAtivo.posto_id}` : "";
    ...
    return `
      <div class="terapeuta-mini ${classe}">
        ...
        <small class="d-block mt-1">${texto}${postoTexto ? `<br><small>${postoTexto}</small>` : ""}</small>
      </div>`;
  }).join("");

  lista.innerHTML = html || "<div class='text-center text-muted w-100'>Nenhum terapeuta online</div>";
}
...
function iniciarAtendimento(id) {
  ...
  select.innerHTML = `<option value="">Escolha o terapeuta...</option>` +
    terapeutas.map(t => `<option value="${t.colaborador_id}">${t.nome_colaborador}</option>`).join("");

  new bootstrap.Modal(document.getElementById("modalIniciarAtendimento")).show();
}

function realmenteIniciarAtendimento() {
  const postoId = document.getElementById("postoIdAtendimento").value;
  const cliente = document.getElementById("nomeCliente").value.trim() || "Cliente Avulso";
  ...
  localStorage.setItem(`atend_${unidadeId}`, JSON.stringify(atendimentosAtivos));
  bootstrap.Modal.getInstance(document.getElementById("modalIniciarAtendimento")).hide();
  atualizarTudo();
}
...
function atualizarRodape() {
  const agora = Date.now();
  const livres = postos.filter(p => 
    !atendimentosAtivos.find(a => a.posto_id == p.posto_id && !a.finalizado) &&
    p.em_manutencao !== 'S' && 
    p.em_limpeza !== 'S' &&
  ...
  const intervalo = postos.filter(p => p.intervalo_fim && p.intervalo_fim > agora).length;
